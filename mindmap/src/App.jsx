...
const STORAGE_KEY = 'algo_mindmap_v4'
...
export default function App() {
  ...
  const [history, setHistory] = useState([])   // [{data, time, count}]
  const [histIdx, setHistIdx] = useState(-1)   // current position
  const mapRef = useRef(null)
  const skipHistory = useRef(false)

  // setData with undo history
  const setData = useCallback((updater) => {
    setDataRaw(prev => {
      const next = typeof updater==='function' ? updater(prev) : updater
      if(skipHistory.current) return next
      setHistory(h => {
        const newEntry = {data: deepClone(next), time: Date.now(), count: countNodes(next)}
        const trimmed = h.slice(0, MAX_HISTORY-1)
  ...
  const redo = useCallback(() => {
    ...
    setHistIdx(newIdx)
    skipHistory.current = true
    setDataRaw(history[newIdx].data)
    setTimeout(()=>{ skipHistory.current=false }, 0)
  }, [histIdx, history])
  ...
  const addChild = useCallback((parentId, labelText=null) => {
    ...
    const node = {id:newId, label: labelText||'Yangi Node', color:'#6366f1', children:[]}
  ...
  const exportPNG = useCallback(() => {
    ...
    oc.toBlob(blob=>{
      const a=document.createElement('a'); a.download='algo-mindmap.png'
...
<style>*{margin:0;padding:0;box-sizing:border-box}body{background:#0d0d0f;color:#f0eee8;font-family:system-ui,sans-serif;min-height:100vh;display:flex;flex-direction:column}header{padding:14px 24px;background:#141416;border-bottom:1px solid #2a2a2e;display:flex;align-items:center;gap:10px}
.logo{font-size:17px;font-weight:700;letter-spacing:.05em}.logo b{color:#6366f1}.meta{font-size:11px;color:#9a9890;margin-left:auto}
main{flex:1;overflow:auto;padding:24px;display:flex;justify-content:center}img{border-radius:12px;border:1px solid #2a2a2e;max-width:100%;height:auto}</style></head>
<body><header><div class="logo">&#x2B21; ALGO <b>MAP</b></div><div class="meta">Eksport: ${now}</div></header>
<main><img src="${img}" alt="ALGO Mind Map" width="${oc.width}" height="${oc.height}"></main></body></html>`
    const blob = new Blob([html],{type:'text/html;charset=utf-8'})
    const a = document.createElement('a'); a.download='algo-mindmap.html'
    a.href=URL.createObjectURL(blob); document.body.appendChild(a); a.click()
    document.body.removeChild(a); setTimeout(()=>URL.revokeObjectURL(a.href),5000)
  }, [])
  ...
  useEffect(()=>{
    ...
    function find(n){if(n.id===selectedId)return n;for(const c of(n.children||[])){const f=find(c);if(f)return f}return null}
  ...
  return (
    <div className="app">
        ...
        <div className="help-overlay" onClick={()=>setShowHelp(false)}>
          <div className="help-box" onClick={e=>e.stopPropagation()}>
            ...
            <div className="help-items">
              ...
              <div><kbd>Drag</kbd><span>Surish</span></div>
              <div><kbd>Click</kbd><span>Tanlash</span></div>
              <div><kbd>Dbl Click</kbd><span>Tahrirlash</span></div>
              <div><kbd>Click ±</kbd><span>Yop/Och</span></div>
              <div><kbd>Space</kbd><span>Tanlanganni yop/och</span></div>
              <div><kbd>N</kbd><span>Node qo'shish</span></div>
              <div><kbd>Del</kbd><span>O'chirish</span></div>
              <div><kbd>Ctrl+Z</kbd><span>Orqaga</span></div>
              <div><kbd>Ctrl+Y</kbd><span>Oldinga</span></div>
              <div><kbd>Esc</kbd><span>Bekor qilish</span></div>
              <div><kbd>Pinch</kbd><span>Mobil zoom</span></div>
              <div><kbd>Tap</kbd><span>Mobil tanlash</span></div>
              <div><kbd>Dbl Tap</kbd><span>Mobil tahrirlash</span></div>
            </div>
        ...
        <div className="history-overlay" onClick={()=>setShowHistory(false)}>
          <div className="history-box" onClick={e=>e.stopPropagation()}>
            <div className="history-header">
              <div className="history-title">🕐 Versiyalar tarixi</div>
              <button className="help-close" onClick={()=>setShowHistory(false)}>✕</button>
            </div>
            {history.length===0 ? (
              <div className="history-empty">Hali o'zgarish yo'q</div>
            ) : (
                ...
                {history.map((h,i)=>(
                  <div key={i} className={`history-item ${i===histIdx?'current':''}`}>
                    ...
                    <div className="history-info">
                      <div className="history-time">{new Date(h.time).toLocaleTimeString('uz-UZ')}</div>
                      <div className="history-nodes">{h.count} ta node</div>
                    </div>
                    {i!==histIdx && (
                      <button className="history-restore" onClick={()=>restoreVersion(i)}>Tiklash</button>
                    )}
                    {i===histIdx && <span style={{fontSize:11,color:'var(--accent)'}}>Joriy</span>}
                  </div>
                ))}
              </div>
            )}
      ...
      <div className="mobile-actions">
        <button onClick={()=>mapRef.current?.collapseAll?.()}>
        ...
        <button onClick={()=>mapRef.current?.expandAll?.()}>
        ...
        <button className={showAI?'active':''} onClick={()=>setShowAI(a=>!a)}>
          ✦ AI
        </button>
      </div>
    </div>
